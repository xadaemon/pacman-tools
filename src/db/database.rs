...
impl Database {
    pub fn open(pth: &Path) -> Result<Self> {
        ...
        Self::decompress(&rdr, &mut dec_data)?;
    ...
    fn decompress(rdr: &File, buf: &mut Vec<u8>) -> Result<usize> {
        let buf_rdr = BufReader::new(rdr);
        let mut zstd_dec = zstd::Decoder::new(buf_rdr)?;

        match zstd_dec.read_to_end(buf) {
            Ok(r) => Ok(r),
            Err(e) => Self::try_gzip(rdr, buf),
        }
    }

    fn try_gzip(rdr: &File, buf: &mut Vec<u8>) -> Result<usize> {
        let buf_rdr = BufReader::new(rdr);
        let mut gzip_dec = GzDecoder::new(buf_rdr);
